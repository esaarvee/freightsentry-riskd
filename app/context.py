...
The Context includes Layer-2 fields (customer_locked_*,
days_since_last_booking, ip_familiarity_tier, impossible_travel,
recipient_cross_customer_count, customer_distinct_ips_30d, etc.). The
recipient cross-customer count requires destination_hmac as a
build_context parameter — caller computes it via signal_helpers.hmac_hex.
...
# case-3a route-unfamiliar derivation parameters.
...
def _outbound_destination_mismatch(
    ...
    """case-3b asymmetric mismatch derivation.
...
def _asn_unfamiliar_for_customer(
    ...
    """case-2 learning-based ASN deviation derivation.
    ...
    (populated by add_observation; decayed
...
async def build_context(
    ...
    *,
    ...
    `tenant_config` populates 5 currency-derived ctx fields
    (shipment_currency + 4 tier thresholds). Cold-start enforcement
    ...
    # fits inside the 30-50ms context-load budget at current cardinality.
    ...
    # case-3b tenant population baseline rarity. The DB
    ...
    # shipment country intermediates (Pydantic Address.country
    ...
    # derivation (`_outbound_destination_mismatch`) reads
    ...
    ctx: dict[str, Any] = {
        ...
        # case-3b signals (brand-new-customer fraud).
        # customer_registered_country is a structured-field passthrough
        # from payload.customer (ISO 3166-1 alpha-2). The outbound check
        # is asymmetric — it fires on a destination-country mismatch
        # matching the Roulottes Lupien attack shape.
        ...
        # tenant population baseline rarity (DB-backed,
        ...
        # Previously rejected — pure dict lookups against the
        # already-loaded baseline; no additional SQL. Rules consume
        # these. email/phone fire only when the CURRENT request
    ...
    # 5 currency-normalized threshold fields. payload.shipment.currency
    # defaults to "USD" per BookingRequest.ShipmentData.
    # resolve_value_caps returns the 4-tier dict, falling back to
    # DEFAULT_VALUE_CAPS["CAD"] (with warning) if the tenant hasn't
    # configured the requested currency. The allowed-list check runs at
    # the endpoint layer — currency reaching here is always permitted.
...
def _modification_direction(
    *,
    ...
    "blocked" (global blocked vectors) is not currently produced;
    the field's Literal type reserves the value for
...
async def build_modification_context(
    ...
    *,
    ...
    Velocity fields (modification_velocity_1h / _24h) are populated via
    count_user_modifications_* below.
    ...
    # Override the synthesized booking's currency to reflect the
    ...
    # currency. Shipment rows that carry no currency yield a synthesized
    # booking with USD via Pydantic default, and this override upgrades
    ...
    # these 2 = 11 total on the same txn connection. Separate-pool
    # parallelism is a future option if the latency budget tightens.
