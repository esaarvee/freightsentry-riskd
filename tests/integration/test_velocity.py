...
async def _seed_shipment(
    ...
    await conn.execute(
        ...
        """,
        tenant_id, customer_id, user_id, request_id, source_ip, booking_ts,
    )
...
async def test_user_hourly_counts_recent_only(
    ...
    await _seed_shipment(db_conn, seeded_tenant, seeded_customer, seeded_user,
                        "fresh-1", "192.0.2.50", now - timedelta(minutes=30))
    await _seed_shipment(db_conn, seeded_tenant, seeded_customer, seeded_user,
                        "old-1", "192.0.2.50", now - timedelta(hours=2))
...
async def test_user_daily_counts_24h_window(
    ...
    for i, delta_hours in enumerate((0.5, 12, 23.5, 25, 48)):
        await _seed_shipment(
            db_conn, seeded_tenant, seeded_customer, seeded_user,
            f"day-{i}", "192.0.2.51", now - timedelta(hours=delta_hours),
        )
...
async def test_user_30d_counts_30day_window(
    ...
    for i, days in enumerate((1, 15, 29, 31, 60)):
        await _seed_shipment(
            db_conn, seeded_tenant, seeded_customer, seeded_user,
            f"30d-{i}", "192.0.2.52", now - timedelta(days=days),
        )
...
async def test_ip_hourly_filters_by_ip(
    ...
    now = datetime.now(tz=UTC)
    await _seed_shipment(db_conn, seeded_tenant, seeded_customer, seeded_user,
                        "match-1", "192.0.2.60", now - timedelta(minutes=15))
    await _seed_shipment(db_conn, seeded_tenant, seeded_customer, seeded_user,
                        "match-2", "192.0.2.60", now - timedelta(minutes=45))
    await _seed_shipment(db_conn, seeded_tenant, seeded_customer, seeded_user,
                        "other-ip", "192.0.2.61", now - timedelta(minutes=10))
...
async def test_ip_daily_24h_window(
    ...
    now = datetime.now(tz=UTC)
    await _seed_shipment(db_conn, seeded_tenant, seeded_customer, seeded_user,
                        "ip-fresh", "192.0.2.70", now - timedelta(hours=12))
    await _seed_shipment(db_conn, seeded_tenant, seeded_customer, seeded_user,
                        "ip-stale", "192.0.2.70", now - timedelta(hours=30))
...
async def test_velocity_scoped_to_tenant(
    ...
    now = datetime.now(tz=UTC)
    await _seed_shipment(db_conn, seeded_tenant, seeded_customer, seeded_user,
                        "in-scope", "192.0.2.80", now - timedelta(minutes=10))
    ...
    other_user = await db_conn.fetchval(
        ...
        other_tenant, other_customer,
    )
    await _seed_shipment(db_conn, other_tenant, other_customer, other_user,
                        "out-of-scope", "192.0.2.80", now - timedelta(minutes=5))
