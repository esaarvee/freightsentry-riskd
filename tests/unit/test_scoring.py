...
Layer 2 tests live in test_scoring_layer2.py. The cases here use a
`_zero_layer2_state` CustomerState that produces `account_prior = 0`
(fully mature + high trust + zero flags), so the assertions exercise
Layer 3 in isolation and the final `noisyOR(0, signal_score) ==
signal_score` collapses to pure-Layer-3 behavior.
"""
...
_zero_layer2_state = CustomerState(
    trust_score=1.0,  # → trust_risk = 0
    account_age_days=365,  # → age_frac saturates
    total_shipments=100,  # → ship_frac saturates → maturity = 1.0 → base_prior = 0
    flagged_count=0,  # → flag_prior = FLAG_WEIGHTS[0] = 0.0
)
