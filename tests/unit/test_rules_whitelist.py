...
# A pinned subset of 11 whitelist fields used as a subset-probe anchor —
# if any of these fields is ever silently removed from the whitelist
# (e.g., during a refactor that drops production code paths they back),
# the membership test below catches it.
...
def test_whitelist_size_matches_current() -> None:
    ...
    additions across the baseline (45) through the case-3b refactor
    + ASN-deviation signal; see docs/history.md for the full
...
def test_whitelist_contains_case_detection_signals() -> None:
    """The case-3a/3b detection primitives must all be present.
    The symmetric ``customer_country_triangle_mismatch`` field was
    DELETED in favor of the asymmetric
    ...
    expected_present = frozenset(
        {
            ...
            # case-3b sophisticated signal.
            "shipment_route_rare_for_tenant",
            # case-2 learning-based deviation.
