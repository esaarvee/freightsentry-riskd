...
- **Canonical lock order — `customers` BEFORE `customer_baselines`.** When a single transaction locks (or `FOR UPDATE`s, or `UPDATE`s) both the `customers` row and the `customer_baselines` row for the same customer, it MUST acquire the `customers` row lock first. The booking path does this naturally (`upsert_customer` precedes the baseline `FOR UPDATE` in `build_context`); the feedback path was fixed to match (it locks `customers FOR UPDATE` before `CustomerBaseline.load(for_update=True)`). Acquiring them in the reverse order deadlocks against a concurrent booking for the same customer (each holds one row and waits for the other → `DeadlockDetectedError`). The general principle is **lock the FK parent before the child** (`customers` is the parent of `customer_baselines`); a future transaction that also locks another child of `customers` (e.g. `users`) derives the right order from this without a second incident. Any new transaction touching both — including the deferred modification-evaluation write path — must follow this order. The lone exception is FK-cascade DELETE in maintenance/cleanup scripts (children-first: `customer_baselines` then `customers`), which is exempt because it never runs concurrently with request traffic.
